...
class Element_El_Woocommerce {
    ...
    public static function add_to_cart_button( $product_id ) {
        if ( ELH_WOOCOMMERCE_ACTIVED ) {
            ...
            if ( $product ) {
                ...
                return sprintf( '<div class="cart item d-flex align-items-center justify-content-center" data-toggle="tooltip" data-placement="top" title="Add to Cart"><a rel="nofollow" data-toggle="tooltip" data-placement="top" title="Add to cart!" href="%s" data-quantity="%s" data-product_id="%s" data-product_sku="%s" class="%s add_to_cart_button action">
                <i class="pe-7s-cart"></i> <span class="action-text">'.esc_html__('Add to cart', 'ta').'</span>
                </a></div>',
    ...
    public static function add_to_cart_button_2( $product_id ) {
        if ( ELH_WOOCOMMERCE_ACTIVED ) {
            $product = wc_get_product( $product_id );
            if ( $product ) {
                $defaults = [
                ...
                return sprintf( '<a rel="nofollow" data-toggle="tooltip" data-placement="top" title="Add to cart!" href="%s" data-quantity="%s" data-product_id="%s" data-product_sku="%s" class="%s add_to_cart_button product-add primary-color d-inline-block text-uppercase">
                '.esc_html__('Add to Cart', 'element-helper').' <span class="pl-1"><i class="fas fa-long-arrow-alt-right"></i></span>
                </a>',
                    esc_url( $product->add_to_cart_url() ),
                    esc_attr( isset( $quantity ) ? $quantity : 1 ),
                    esc_attr( $product->get_id() ),
    ...
    public static function add_to_cart_button_text( $product_id ) {
        if ( ELH_WOOCOMMERCE_ACTIVED ) {
            ...
            if ( $product ) {
                ...
                return sprintf( '<a rel="nofollow" href="%s" data-quantity="%s" data-product_id="%s" data-product_sku="%s" class="%s add_to_cart_button product-add product-add-2">'.esc_html__('Add to cart', 'elementhelper').'t</a>',
                    esc_url( $product->add_to_cart_url() ),
                    esc_attr( isset( $quantity ) ? $quantity : 1 ),
                    esc_attr( $product->get_id() ),
                    esc_attr( $product->get_sku() ),
    ...
    public static function yith_wishlist( $product_id ) {
        ...
        if ( ELH_WISHLIST_ACTIVED ) {
            ...
            $markup .= '<a href="' . $link . '/wishlist/view/" rel="nofollow" class=" ' . $cssclass . '"><i class="yith-wcwl-icon pe-7s-like"></i> <span class="action-text">'.esc_html__('View wishlist', 'ta').'</span></a>';
            $markup .= $img;
            $markup .= '</div>';
    ...
    public static function ta_get_price() {
        ob_start();?>
            <?php print ta_get_price_html();?>
        <?php
        return ob_get_clean();
    }

    public static function ta_get_price_html() {
        global $product;
    ...
    public static function quick_view_button( $product_id ) {
        if ( TA_QUICK_VIEW_ACTIVED ) {
            global $product;
            $product = wc_get_product( $product_id );
            $button = '';
            if ( $product_id ) {

                $button = '<a href="#" class="yith-wcqv-button" data-product_id="' . esc_attr( $product_id ) . '" data-toggle="tooltip" data-placement="top" title="Quick View!"><i class="pe-7s-look"></i> <span class="action-text">'.esc_html__('Quick View', 'ta').'</span></a>';
    ...
    public static function compare_button( $product_id ) {
        if ( TA_COMPARE_ACTIVED ) {
            global $product;
            $product = wc_get_product( $product_id );
            $ta_compare = '';
            if ( $product_id ) {
                $ta_compare = '<button class="woosc-btn woosc-btn-'.$product_id.'" data-id="'.$product_id.'"><i class="pe-7s-repeat"></i></button>';
            }
