...
if(!class_exists("MagezixLicenseBase")) {
	class MagezixLicenseBase {
    	public $key = "3ABF3EE10E5CCE4D";
    	private $product_id = "1";
    	private $product_base = "magezix-license";
    	private $server_host = "https://themexriver.com/wp/magezix/wp-json/licensor/";
		...
		function __construct($plugin_base_file='')
		{
			...
			if($this->hasCheckUpdate) {
				if(function_exists("add_action")){
					add_action( 'admin_post_magezix-license_fupc', function(){
					...
					if($this->isTheme){
						...
						add_filter( 'plugin_row_meta', function($links, $plugin_file ){
							if ( $plugin_file == plugin_basename( $this->pluginFile ) ) {
								$links[] = " <a class='edit coption' href='" . esc_url( admin_url( 'admin-post.php' ) . '?action=magezix-license_fupc' ) . "'>Update Check</a>";
							}
							return $links;
						}, 10, 2 );
