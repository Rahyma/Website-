...
class magezix_post_list_tab extends Widget_Base {
	...
	public function get_title() {
		return esc_html__( 'Post Tab List', 'magezix-core' );
	}
	...
	protected function register_controls() {
		...
		$this->add_control(
			...
			[
				...
				'size_units' => [ 'px' ],
				'selectors' => [
					'{{WRAPPER}} .tab-post__item.tx-post .post-thumb img' => 'height: {{SIZE}}{{UNIT}};',
				],
			]
		);
    ...
    ?>
		<div class="tab-post__wrap ml-none-30 mllg-none-100">
                <ul class="tab-post__nav nav nav-tabs" id="myTab" role="tablist">
                    <?php foreach($settings['magezix_list_tab'] as $item):?>
                        <li class="nav-item" role="presentation">
                            <button class="nav-link <?php if('yes' == $item['is_active']){echo esc_attr('active');}?>" id="magezix-tab-item-<?php echo esc_attr($item['_id']);?>" data-bs-toggle="tab" data-bs-target="#magezix-home-<?php echo esc_attr($item['_id']);?>" type="button" role="tab" aria-controls="home" aria-selected="true"><?php \Elementor\Icons_Manager::render_icon( $item['tbicon'], [ 'aria-hidden' => 'true' ] ); ?><?php echo esc_html($item['tab_title']);?></button>
                        </li>
                    <?php endforeach;?>
                </ul>
				...
				?>
                    <div class="tab-pane animated fadeInUp <?php if('yes' == $item['is_active']){echo esc_attr('show active');}?>" id="magezix-home-<?php echo esc_attr($item['_id']);?>" role="tabpanel" aria-labelledby="magezix-tab-item-<?php echo esc_attr($item['_id']);?>">
						<?php
						if ( $query->have_posts() ) {
						$iten_number = 0;
...
Plugin::instance()->widgets_manager->register( new magezix_post_list_tab() );