...
function theme_enqueue_multiple_scripts() {
    $scripts = [
        'mlib.js',
        'functions.js',
    ];
    ...
    foreach ($scripts as $script) {
        wp_enqueue_script(
            sanitize_title($script), // Handle
            get_template_directory_uri() . '/assets/js/' . $script, // Source
            array(), // Dependencies
            true // Load in footer
        );
    }
}
add_action('wp_enqueue_scripts', 'theme_enqueue_multiple_scripts');
// custom coding


// Enqueue JavaScript for sorting
// SHORTCODE FUNCTION
function blog_grid_layout_ajax() {
    ...
    echo '<div class="blog-grid-wrapper">';
    if ($query->have_posts()) :
        while ($query->have_posts()) : $query->the_post(); ?>
            <div class="blog-card">
                ...
                <div class="card-content">
                    <div class="card-meta">
                        <span class="author"><?php echo get_avatar(get_the_author_meta('ID'), 24); ?> by <span class="author-name"><?php the_author(); ?></span></span>
                        <span class="date"><?php echo get_the_date(); ?></span> .
                        <span class="comments"><?php comments_number('No Comments', '1 Comment', '% Comments'); ?></span>
                    </div>
                    <p class="card-excerpt"><?php echo wp_trim_words(get_the_excerpt(), 9); ?></p>
                    <a href="<?php the_permalink(); ?>" class="main-button3">Read More</a>
                </div>
            </div>
        <?php endwhile;
    else :
...
add_shortcode('blog_grid_layout', 'blog_grid_layout_ajax');
// ENQUEUE SCRIPT
function enqueue_blog_sort_scripts() {
    wp_enqueue_script('jquery');

    wp_enqueue_script('blog-sort-ajax', get_template_directory_uri() . '/assets/js/functions.js', array(), true);

   
	wp_localize_script('blog-sort-ajax', 'blog_sort_obj', array(
        'ajax_url' => admin_url('admin-ajax.php'),
        'nonce'    => wp_create_nonce('blog-sort-nonce')
    ));
}
add_action('wp_enqueue_scripts', 'enqueue_blog_sort_scripts');


// AJAX HANDLER
function blog_sort_ajax_handler() {
    echo blog_grid_layout_ajax();
    wp_die();
}
add_action('wp_ajax_blog_sort_ajax', 'blog_sort_ajax_handler');
add_action('wp_ajax_nopriv_blog_sort_ajax', 'blog_sort_ajax_handler');
